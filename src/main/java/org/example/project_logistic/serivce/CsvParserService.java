...
public class CsvParserService {
    ...
    // Форматтер для времени (поддерживает HH:mm и HH:mm:ss)
    ...
    public List<Order> parseOrders(MultipartFile file) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            for (CSVRecord record : csvParser) {
                ...
                order.setRequiredExperienceLevel(Integer.parseInt(getValue(record, "required_experience_level", "1")));                orders.add(order);
            }
        }
        return orders;
    }
    ...
    public List<Driver> parseDrivers(MultipartFile file) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            for (CSVRecord record : csvParser) {
                ...
                // Начальные значения для алгоритма (текущее положение — склад)
                driver.setCurrentStops(0);
                driver.setLastLat(null);
                driver.setLastLon(null);
    ...
    private String getValue(CSVRecord record, String columnName, String defaultValue) {
        return record.isMapped(columnName) ? record.get(columnName) : defaultValue;
    }
}
